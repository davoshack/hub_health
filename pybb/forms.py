...
class PostForm(forms.ModelForm):
    name = forms.CharField(label=ugettext_lazy('Subject'), widget=forms.TextInput(attrs={'class': 'form-control','style':'width:50%'}))
    login = TextInput(attrs={'maxlength': 125, 'class': 'form-control','style':'width:50%'}),
    # poll_type = forms.TypedChoiceField(label=ugettext_lazy('Poll type'), choices=Topic.POLL_TYPE_CHOICES, coerce=int, widget=forms.Select(attrs={'class': 'form-control', 'style': 'width:50%'}))
    ...
    class Meta(object):
        ...
        widgets = {
            ...
            'body': Textarea(attrs={'cols': 50, 'rows': 5,'class': 'form-control','style':'width:50%'}),
