    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not is_safe_url(url=redirect_to, host=request.get_host()):
                ...
                date_birth = ProfileUser.objects.filter(user_id=form.get_user_id)
                if date_birth.count() == 1:
                    redirect_to = resolve_url(settings.LOGIN_REDIRECT)
                else:
                    redirect_to = resolve_url(settings.LOGIN_REDIRECT_PROFILE_SETTING)
...
def password_reset(request, is_admin_site=False,
    ...
    if post_reset_redirect is None:
        ...
        post_reset_redirect = resolve_url(password_reset_done)
    if request.method == "POST":
