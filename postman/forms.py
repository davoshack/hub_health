...
class BaseWriteForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'subject': TextInput(attrs={'maxlength': 125, 'class': 'form-control','style':'width:50%'}),
            'body': Textarea(attrs={'cols': 50, 'rows': 6,'class': 'form-control','style':'width:100%'}),
        }
...
class WriteForm(BaseWriteForm):
    ...
    recipients = CommaSeparatedUserField(label=('Destinatario'),widget=TextInput(attrs={'maxlength': 125, 'class': 'form-control','style':'width:50%'}))
    #recipients = CommaSeparatedUserField(label=('Destinatario'),help_text='ingresa el correo del destinatario')
