...
def email_visitor(object, action, site):
    """Email a visitor."""
    email('email_visitor_subject.txt', 'email_visitor.txt', [object.email], object, action, site)
...
def notify_user(object, action, site):
    ...
    if notification:
        ...
        if not DISABLE_USER_EMAILING and user.email and user.is_active:
            email('email_user_subject.txt', 'email_user.txt', [user.email], object, action, site)
